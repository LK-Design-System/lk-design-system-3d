...
One immutable layer owns markers captured in one frame at one optional timestamp. The caller supplies the frame-to-scene transform, normally from `@lk-robotics/design-system-3d-tf`; transport, ROS message actions, product retention policy, and application UI remain outside this package.
